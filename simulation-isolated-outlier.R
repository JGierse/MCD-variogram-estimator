...
load(file = paste0("Isolated/data_grid", 
...
res <- future_lapply(1:1000, function(i){
  sim.it(data$data[[i]], data$grid, hmaxs[[combs.iso$hmax[[ind]]]], cp = 1)
}, future.seed = TRUE)
...
for(ind in 1:nrow(combs.iso)){ # for each scenario
  ...
  for(r in 1:4){ # for each direction
    ...
    t(total) -> kons.mean[, r, ,  combs.iso$amount[ind]-1, combs.block$param.outlier[ind]-1]
    t(Bias) -> kons.bias[, r, ,  combs.iso$amount[ind]-1, combs.block$param.outlier[ind]-1]
    t(MSE) -> kons.sqrtMSE[, r, ,  combs.iso$amount[ind]-1,  combs.block$param.outlier[ind]-1]
  }
}
